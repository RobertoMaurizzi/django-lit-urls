...
class UrlParserTestCase(unittest.TestCase):
    def test_url_patterns(self):
        ...
        self.assertEqual(
            _url_to_js_func(r"editor/activity-manager/<slug:org_id>/"), (r"editor/activity-manager/${org_id}/", ("org_id",))
        )
        ...
        self.assertEqual(
            ...
            (r"activity/${activity_pk}/endorsement/${org_pk}/", ("activity_pk", "org_pk")),
        )

        # Positional arguments are replaced with `v_x`
        self.assertEqual(_url_to_js_func(r"activity/(?:[\w-]*\/{0,1})api/"), (r"activity/${v_1}api/", ("v_1",)))
...
class UrlModelTestCase(unittest.TestCase):
    def test_url_model(self):
        ...
        self.assertEqual(test_case.as_class_prop, "villageCourtDetail (pk) { return `/en/dims/village-court/${pk}/` }")
        self.assertEqual(test_case.as_arrow_func, "villageCourtDetail = (pk) => `/en/dims/village-court/${pk}/`;")
        self.assertEqual(
            test_case.as_arrow_func_property, "villageCourtDetail: (pk) => `/en/dims/village-court/${pk}/`"
        )
        self.assertEqual(
            test_case.as_function, "function villageCourtDetail(pk){ return `/en/dims/village-court/${pk}/` }"
        )
